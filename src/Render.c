...
static void Render_generateVagine(int frame, ws2812_pixel_t *pixels, int hue, bool direction) {
    hsv_t data[WIDTH][HEIGHT];
    Render_fillBlack(data);
    int localFrame = direction ? frame % 360  : 360 - frame % 360 - 1;
    ...
    for (int x = 0; x < WIDTH; x++)
    {
        for (int y = 0; y < HEIGHT; y++)
        {
            ...
            l_data.s = 1;
            l_data.v = 1;
            // if(colorQue[0] == s_vaginePixels[x][y]) {
                int pos = 0;
...
static void Render_render(int frame, ws2812_pixel_t *pixels) {
    ...
    case MATRIX_MODE:
        ...
        break;
    case VAGINE_MODE:
        Render_generateVagine(frame, pixels, l_params->currentHue, l_params->currentDirection);
        break;
    default:
        Render_generateRainbow(
            frame,
